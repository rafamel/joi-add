...
describe(`- .addFn()`, () => {
    ...
    describe(`- Message & noKey & isExplicit`, () => {
        test(id(1) + `Defaults to noKey = true when message`, () => {
            const val = Joi.number().addFn(() => false, 'Some message').label('Some');
            const { error } = Joi.validate(1, val);
        ...
        test(id(2) + `Defaults to noKey = false when no message`, () => {
            const val = Joi.number().addFn(() => false).label('Some');
            const { error } = Joi.validate(1, val);
        ...
        test(id(3) + `Message + noKey = false`, () => {
            const val = Joi.number().addFn(() => false, 'Some message', false).label('Some');
            const { error } = Joi.validate(1, val);
        ...
        test(id(4) + `No message + noKey = true`, () => {
            const val = Joi.number().addFn(() => false, null, true).label('Some');
            const { error } = Joi.validate(1, val);
