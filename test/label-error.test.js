...
describe(`- addLabel()`, () => {
    ...
    describe(`- Scalar labeling`, () => {
        ...
        test(id(4) + `addLabel does not exist if .label() has been later used`, () => {
            const val = Joi.number().min(5)
                .addLabel('labelB')
                .label('labelA');
            const { error } = Joi.validate(2, val);
        ...
        test(id(6) + `addLabel persists when using .onError()`, () => {
            const val = Joi.number().min(5).addLabel('labelA').onError(x => x);
            const { error } = Joi.validate(2, val);
