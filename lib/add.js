...
module.exports = function (joi, type) {
    return function validate(params, value, state, options) {
        ...
        if (typeof schema === 'function') {
            ...
            const { error } = joi.validate(schema, joi.object().schema());
            if (error) throw Error('The function passed to .add() must return a Joi schema.');
        }
        ...
        if (ans.error) {
            ...
            err.new = this.createError(
                (config.inheritErrorTypes) ? err.old.type : `${type}.add`,
